...
void eigs( SMatrixCRS<double> &kCRS, SMatrixCRS<double>& mCRS,
		int nsize,
		int nModes,
		Matrix<double> & eigvalue_in,
		Matrix<double> & eigvector_in,
	    int ncv=0	);
...
Matrix<double> gausselim(SpMatrix& A, const Matrix<amc_real>& b);
...
Matrix<double> sparseSOR(SpMatrix* A, Matrix<double> *b, Matrix<double> *xold, double tol, int maxiter, double w=1.7, char check='n');
