...
const int WIDTH = 13;		// width of field for printing matrices
...
public:
	...
	void mprint() const
	{
		...
		{
			for(amc_int j = 0; j < ncols; j++)
				std::cout << std::setw(WIDTH+2) << std::setprecision(WIDTH/2) << elems[i*ncols+j];
			std::cout << std::endl;
		}
	}
	...
	/// Reads matrix from file, the first line is number of rows and columns
	...
	// read matrix, there is no first line that tells rows and columns
	void fread(std::ifstream& infile, int rows, int cols)
	{
		//infile >> nrows; infile >> ncols;
		nrows = rows;
		ncols = cols;
	...
	void importdata( const std::string filename )
    {
		...
		char delimeter; // not implement for not blank delimeter
		T d1;
		std::vector<T> ldata;
		ncols=0;
		while (iss>>d1)
	...
	void addMatrix( Matrix<T> subA, int* indx, int* indy = NULL )
	{
		...
		for(int i =0; i<subA.rows(); i++)
			for(int j=0; j<subA.cols(); j++)
				this->operator()(indx[i], indy[j]) += subA(i,j);
	}

	void addVector( Matrix<T> insertV, int* inds)
	{
		assert( nrows == 1 || ncols == 1);
		for(int i =0; i<insertV.size; i++)
				this->operator()(0,inds[i]) += insertV(0,i);
	}
